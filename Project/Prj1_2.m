...
%  |   1.1     | H.F.        | 2017.4.22 | 
%  |   2.0     | H.F.         | 2020.4.18 | remodeling due to total wrong
%--------------------------------------------------------------------------
close all;
clear;
...
theta=1e2 ; % tetR2与 DNA binding sites的反应平衡常数(dissociation constant)
t=3e3;  % 时间长度,2天
TtetR=c*t; % t时间内tetR总生产量
n=3.5; % Hill coefficient
B=2.1;

% The GFP expression after 2 day
tetR2=k1*(TtetR*TtetR)./(k2*Dox) - (exp(-t*k2))./(k2*Dox);
pCMV2=(theta.^n)./(theta.^n+tetR2.^n); % 
GFP=(B*pCMV2+0.028)*t;% GFP expression. Leakage always exist in expression.
figure(1);
loglog(Dox,GFP,'or');
...
% When the concentration of Dox is 1
figure(2);
time=[0:1:100];
